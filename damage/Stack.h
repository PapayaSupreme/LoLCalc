...
protected:
    ...
    std::multimap<uint16_t, const Effect*> effects;
    const bool stop_at_max_count;
public:
    Stack(std::string name, EffectTrigger effect_trigger,
        std::multimap<uint16_t, const Effect *> effects, bool stop_at_max_count);

    [[nodiscard]] std::string get_name() const noexcept;
    [[nodiscard]] EffectTrigger get_effect_trigger() const noexcept;
    [[nodiscard]] std::multimap<uint16_t, const Effect *> get_effects() const noexcept;
